...
\paragraph{Site location(s)} These are latitude and longitude pairs for each site of interest. For regional analyses, ground-shaking is either characterized directly at the location of each asset or a carefully designed intermediate grid is used and the site-specific results are determined by interpolation between the grid nodes. The direct approach is used when the number of assets is relatively small \citep[see e.g.,][]{padgett2010regional} while the interm

(e.g.,  hazard characteristics at the nodes of a carefully designed grid are used, and the site-specific results are determined by interpolation (e.g., 
