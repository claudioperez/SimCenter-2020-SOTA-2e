...
This chapter reviews the above methods, the data they use, and the tools that have implemented them. The recent book by \cite{baker2021seismic} provides more details about the characterization of ground shaking and the methods available for probabilistic seismic hazard assessment. 
...
Conventional, empirical GMMs are expected to be replaced by non-ergodic models that can consider local site and path information when available \citep[e.g.,][]{abrahamson2019probabilistic}. In areas with abundant information, such models are expected to reduce the variability in the ground motion and provide a more realistic prediction of the seismic hazard. Because such detailed information is not available in every location, the models need to account for site-specific aleatory and epistemic uncertainty and acknowledge that the uncertainty in the estimated hazard is not uniform in space. This enhanced treatment of uncertainty will also require an expansion of existing logic trees to represent the center, body, and range of the input parameters \citep{gerstenberger2020seismic}.
The rigorous combination of multiple empirical GMMs \citep[e.g.,][]{goulet2017ngaeast} is also a promising direction that would allow the consideration of model uncertainty in the estimated hazard. Other sources of uncertainty that affect the seismic source model, or the physical model of the crust that is used to estimate soil amplification, for example, are also important to quantify and propagate through the analyses. 
It is also increasingly recognized that there is long-term variability in the rate of earthquakes and some nations have already adopted time-dependent, non-Poissonian models to estimate the hazard rates more accurately. 

As our ability to compute ground motions to ever higher frequencies increases as a result of rapid computational advancements, we are outstripping our knowledge of the subsurface geology at fine scale. A challenge will be how to address geologic model uncertainties.

... simulated ground motions at higher frequencies and in softer soils; site amplification with uncertainty; information about site characteristics; Cascadia issues; time independent to time dependent
