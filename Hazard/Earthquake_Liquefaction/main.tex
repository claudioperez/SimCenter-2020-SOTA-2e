...
In 1998, a consensus was reached within the geotechnical community on the use of an empirical stress-based approach for liquefaction triggering assessment called the ``simplified method,'' first developed in \cite{seed1971simplified}. This method is still commonly used in practice today \citep{youd2001liquefaction,national2016state}.
...
\cite{seed1971simplified} proposed a simplified equation, based on Newton's second law, to compute a representative CSR for a given earthquake magnitude. This model was later revised by \cite{idriss1999update}, \cite{cetin2004nonlinear}, \cite{idriss2008soil}, and \cite{boulanger2014cpt}.

%NOTE: reword?
The most common approaches used in practice to compute CRR are based on geotechnical field data, e.g., CPT, SPT, and $V_s$.
The most commonly used relationships to estimate CRR from a CPT profile are those developed by \cite{robertson1998evaluating}, \cite{moss2006cpt}, \cite{idriss2008soil}, and \cite{robertson2009interpretation}.
The most commonly used relationships to estimate CRR from SPT blow count are those proposed by \cite{youd2001liquefaction}, \cite{boulanger2014cpt}, and \cite{cetin2018sptbased}. 
The most commonly used relationships to estimate CRR from a $V_s$ profile are those developed by \cite{andrus2000liquefaction} and \cite{kayen2013shearwave}.
