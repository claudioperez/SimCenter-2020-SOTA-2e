...
Better results can be attained using a perfectly match layer (PML), which is an artificial absorbing layer for wave equations commonly used to truncate computational regions in numerical methods to simulate problems with open boundaries, especially in finite-difference and finite-element methods \citep{Zhang19}. PML's are designed so that waves incident upon the PML do not reflect back to the medium at the interface. In general, PMLs have been shown to produce better results than LK dashpots.
...
\item Performing adequate soil-structure interaction (SSI) for major facilities continues to be a challenge and a gap in geotechnical earthquake engineering. In general, older codes (1970s vintage), continue to be the preferred option. Although very useful, these codes are based on simplified assumptions, and do not offer real HPC capabilities, so one is continually forced towards model-size-reduction compromises.
